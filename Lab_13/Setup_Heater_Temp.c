...
In_Heater_Temp_Setup = 1;
    ...
    while (In_Heater_Temp_Setup == 1)
    {
		...
		// add code to handle 'Play/Pause', 'Minus', 'Plus' and 'EQ'
                if (found == Play_Pause) Do_Save_New_Heater_Temp();// add code here to check if 'Play/Pause' then use Do_Save_New_Time();                
                if (found == Minus) Decrease_Heater_Temp();// add code here to check if 'Minus' then use Decrease_Time();
                if (found == Plus) Increase_Heater_Temp();// add code here to check if 'Plus' then use Increase_Time();
...
void Increase_Heater_Temp()
{
    ...
    switch(Select_Heater_Temp_Field)
    {
        case 0:
            setup_heater_set_temp++;
            if(setup_heater_set_temp > 120)
            {
                setup_heater_set_temp = 120;
            }
            break;
        default:
        break;
...
void Decrease_Heater_Temp()          
{
    ...
    {
        case 0:
            setup_heater_set_temp--;                                    // decrement heater set temp by one 
            if (setup_heater_set_temp < 50) 
            {
                setup_heater_set_temp = 50;
            }  // limit the minimum set temp to 50
            break;
...
void Update_Setup_Heater_Screen(void)
{
    ...
    char dc_char3 = setup_heater_set_temp%10; // THIS IS PROBABLY WRONG 
