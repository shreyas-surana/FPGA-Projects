...
void Test_Alarm()
{
    if (alarm_mode == 0 && ALARMEN == 1)                                        // if alarm mode is off and alarm switch is on  
    {
            DS3231_Turn_On_Alarm();                                             // turn alarm on
            alarm_mode = 1;  
            Clear_RGB(); // set alarm mode to on
            MATCHED = 0;
    }
    else if (alarm_mode == 1 && ALARMEN == 0)                                   // if alarm mode is on and alarm switch is off   
    {
            DS3231_Turn_Off_Alarm();                                            // turn alarm off
            alarm_mode = 0;                                                     // set alarm mode off 
            Deactivate_Buzzer();                                                // stop buzzing
            do_update_pwm(duty_cycle);  
            Clear_RGB(); // update the pwm for the dc, in order to control RPM(speed) of fan (heater)                                      
    }
    else if (alarm_mode == 1 && ALARMEN == 1)                                   // if both alarm mode and alarm switch are on 
    {
        if (RTC_ALARM_NOT == 0)                                                 // and RM is 0
        {
            MATCHED = 1;                                                        // set match to 1
            Activate_Buzzer();                                                  // turn in buzzer
            do_update_pwm(duty_cycle);                                          // update the pwm for the dc, in order to control RPM(speed) of fan (heater)                            
            Set_RGB_Color(color);                                               // turn on alarm time RGB
        }
        if (MATCHED == 1)                                                       // if alarm time and time match up (equal each other)
        {
            Set_RGB_Color(color);                                               // turn on alarm time RGB
            color++;                                                            // change color of RGB from off, red, yellow,blue, purple, cyan, and white                                                  
            if(color==8)                                                         // once a cycle of the colors have been completed
            {
                color = 0;                                                      // it will be reset back to no color to repeat cycle all over again 
            }
            if (volt > 3)                                                       // if voltage of photo resistor is more than 3 (cover my pen cap)
            {
                Deactivate_Buzzer();                                            // buzzing will stop
                Clear_RGB();                                                    // all RBG's will turn off (no color)
                do_update_pwm(duty_cycle);                                      // update the pwm for the dc, in order to control RPM(speed) of fan (heater)                 
                DS3231_Turn_Off_Alarm();                                        // alarm will turn off
                alarm_mode=0;                                                   // alarm mode will be set off
                MATCHED = 0;  
                //ALARMEN = 1;// time and alarm time will no longer be considered the same, so matched is set off
            }
        }
    }
}
